...
void neuron_connect_neurons(Neuron *firstNeuron, Neuron *secondNeuron)
{
    ...
    if (secondNeuron->dendrites == NULL)
    {
        ...
        secondNeuron->weights = malloc(secondNeuron->numberDendrites * sizeof(double));
    }
    else
    {
        ...
        double *currentWeights = secondNeuron->weights;
        double *newWeights = malloc(secondNeuron->numberDendrites * sizeof(double));
...
void neuron_update_weight(Neuron *neuron, float learningRate, double error)
{
    double out = neuron->axonValue;
